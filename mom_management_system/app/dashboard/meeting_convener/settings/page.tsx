...
import { Save, Loader2, User, Palette, Moon, Sun, Smartphone, ShieldCheck, Mail, Fingerprint, Camera, ChevronRight } from 'lucide-react';
  ...
  return (
    <div className="max-w-4xl w-full mx-auto space-y-6 pb-20 pt-2 px-4">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 py-2">
        <div>
          <h1 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight">Convener Settings</h1>
          <p className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mt-0.5">Management Profile & Preferences</p>
        </div>
        <button
          onClick={handleSave}
          disabled={saving}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          ...
          <section className="bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800 rounded-[2rem] overflow-hidden shadow-sm">
            <div className="p-8 pb-0 flex flex-col items-center text-center">
              <div className="relative group">
                ...
                <div className="absolute -bottom-2 -right-2 bg-black dark:bg-white text-white dark:text-black w-8 h-8 rounded-xl border-4 border-white dark:border-slate-900 flex items-center justify-center">
                  <ShieldCheck size={14} />
                </div>
              </div>
              <h2 className="mt-6 text-xl font-bold text-slate-900 dark:text-white">{profile.name}</h2>
              <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mt-1">Authorized Meeting Convener</p>
            </div>
        ...
        <div className="lg:col-span-4 space-y-6">
          <section className="bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800 rounded-[2rem] p-6 shadow-sm">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                ...
                <p className="text-[9px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">Theme</p>
              </div>
            </div>
          ...
          <section className="bg-slate-50 dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800 rounded-[2rem] p-6 shadow-sm">
            <h4 className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-4">Management Status</h4>
            <div className="space-y-4">
              <div className="flex items-center justify-between group cursor-pointer text-emerald-600">
              ...
              <div className="pt-4 mt-4 border-t border-slate-100 dark:border-slate-800">
                 <p className="text-[9px] font-black text-slate-300 dark:text-slate-600 uppercase">Registry Status</p>
                 <p className="text-[10px] font-bold text-slate-500">Meeting Convener Access</p>
              </div>
            </div>
          </section>
        </div>
      ...
      <div className="fixed bottom-8 right-8 z-50">
        {message && (
          <div className="bg-slate-900 dark:bg-white dark:text-slate-900 text-white px-6 py-4 rounded-2xl shadow-2xl flex items-center gap-3 animate-in fade-in slide-in-from-bottom-4">
            <ShieldCheck size={18} className="text-emerald-400" />
            <p className="text-[10px] font-black uppercase tracking-widest">{message}</p>
          </div>
        )}
        {errorHeader && (
          <div className="bg-rose-600 text-white px-6 py-4 rounded-2xl shadow-2xl flex items-center gap-3 animate-in fade-in slide-in-from-bottom-4">
            <p className="text-[10px] font-black uppercase tracking-widest">{errorHeader}</p>
          </div>
        )}
      </div>
    </div>
